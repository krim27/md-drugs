...
for k, v in pairs (Consumables.Items) do QBCore.Functions.CreateUseableItem(k, function(source, item)
	...
	if v.statval ~= nil and v.statval == nil then v.statval = Consumables.defaultstatval end
		 TriggerClientEvent("wrp-drugs:client:consumedrugs", source, v.time, v.effect, v.anim, v.progressbartext, v.status,  v.statval, k) 
	end)
end

RegisterNetEvent('wrp-drugs:server:removeconsum', function(item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
		Player.Functions.RemoveItem(item, 1) 
end)    

RegisterNetEvent('wrp-drugs:server:updatestatus', function(stat, statval)
